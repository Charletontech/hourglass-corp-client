...
function getWalletBalance() {
  fetch(
    `https://hourglass-corp-server.onrender.com/get-balance?phone=${userData.phone}`
  )
    .then((res) => {
      res.json().then((data) => {
        if (res.ok) {
...
async function verifyTransaction(reference, fundAmount, phone) {
  ...
  fetch(
    "https://hourglass-corp-server.onrender.com/verify-payment",
    fetchOptions
  )
    .then((res) => {
      if (res.ok) {
...
function submitNinValidation(ninValidationType, nin) {
  ...
  fetch(
    "https://hourglass-corp-server.onrender.com/nin-validation",
    fetchOptions
  )
    .then((res) => {
      res.json().then((data) => {
...
function submitSuspendedNin(nin) {
  ...
  fetch(
    "https://hourglass-corp-server.onrender.com/suspended-nin",
    fetchOptions
  )
    .then((res) => {
      res.json().then((data) => {
...
async function handleDataModificationBtn() {
  const { value: dataModificationType } = await Swal.fire({
    ...
    inputOptions: {
      name: "Name (₦15,000)",
      phoneNumber: "Phone Number (₦15,000)",
      DateOfBirth: "Date Of Birth (₦43,000)",
    },
  ...
  if (dataModificationType) {
    const { value: formValues } = await Swal.fire({
      ...
      html:
        ...
        `<input id="swal-input15" class="swal2-input" type="file" accept="image/*" placeholder="Upload Picture">` +
      ...
      cancelButtonColor: "#00c4cc",
      preConfirm: () => {
        const pictureFile = document.getElementById("swal-input15").files[0];
        if (!pictureFile) {
          Swal.showValidationMessage("You must upload a picture!");
          return false;
        }
        return {
    ...
    if (formValues) {
      ...
      for (const key in formValues) {
        if (key === "pictureFile") {
          formData.append("picture", formValues[key]);
        } else {
      ...
      fetch(
        "https://hourglass-corp-server.onrender.com/data-modification",
        fetchOptions
      )
        .then((res) => {
          res.json().then((data) => {
